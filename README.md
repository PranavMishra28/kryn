...
The pinned stack is **OpenCode 2.0.10**, **oMLX 0.6.4**, **Qwen3.5-9B-6bit**, **Playwright MCP 0.0.82** and keyless Exa search. Public v0.1.10 uses a **49,152-token context**. This unreleased source candidate configures **65,536 tokens** for private qualification; [tier measurements](evals/history/2026-09-25-context-tiers/qualification.json) support one near-limit cold and warm reasoning retrieval, but sustained agent quality has not passed. Both profiles use an **8,192-token output limit**, **16 GiB model memory ceiling** and one active generation. Exact model hashes and tool settings are in [the accepted profile](setup/accepted-profile.json) and [the client template](setup/opencode.template.json).

Automatic compaction reserves room for output: the candidate's 65,536-token window leaves roughly 57,344 tokens for active context before native estimation triggers a summary (public v0.1.10: 40,960). It retains bounded recent user context alongside a structured checkpoint. The complete session history and written files remain on disk; summaries are not lossless, so the agent is instructed to reconcile them with files and check results. Compaction uses a separate 2,048-token fast summary budget. The 96K and 128K near-limit reasoning probes hit sustained host memory warnings and were canceled; neither is a supported daily setting.
