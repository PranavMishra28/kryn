...
The pinned stack is **OpenCode 2.0.10**, **oMLX 0.6.4**, **Qwen3.5-9B-6bit**, **Playwright MCP 0.0.82** and keyless Exa search. Public v0.1.10 uses a **49,152-token context**. The current unreleased source and owner installation use **65,536 tokens** at the owner's request, with an **8,192-token output limit**, **16 GiB model memory ceiling** and one active generation. One synthetic near-limit cold/warm reasoning check passed, but three installed OpenCode agent starts hit sustained host memory pressure before editing. The 49K private candidate remains the rollback option; 64K is experimental under this Mac's current workload. [Tier measurements and failure evidence](evals/history/2026-09-25-context-tiers/qualification.json) explain the status. Exact model hashes and tool settings are in [the accepted profile](setup/accepted-profile.json) and [the client template](setup/opencode.template.json).

Automatic compaction reserves room for output: the installed 65,536-token setting leaves roughly 57,344 tokens for active context before native estimation triggers a summary (49K fallback: 40,960). It retains bounded recent user context alongside a structured checkpoint. The complete session history and written files remain on disk; summaries are not lossless, so the agent is instructed to reconcile them with files and check results. Compaction uses a separate 2,048-token fast summary budget. The 64K installed agent trial and 96K/128K near-limit reasoning trials hit sustained host memory warnings; keep the guard active and use `kryn rollback` if 64K sessions abort under normal use.

The unreleased source candidate adds a bounded read-only view of current Git state and up to four files named in the native checkpoint. It records a Git/content fingerprint at compaction and warns when that checkpoint's workspace state has changed, including after a restart. It also keeps the first and three latest user requests in private state and flags a checkpoint that plainly denies an existing task. The request record is intentionally bounded and may omit criteria; the complete native transcript stays on disk outside the prompt. A completed compaction is only a structural event, not proof that its summary is accurate. Public v0.1.10 retains the previous behavior; long-workflow qualification has not passed.

In the [private Task12 trial](evals/history/2026-09-25-context-candidate/qualification.json), two native compactions completed around a restart, but the first checkpoint falsely said there was no task. Backend and UI turns timed out; independent Chrome verified several flows but found broken exact filtering and horizontal overflow at 375 px. The frozen suite graded the run FAIL. This candidate is not a qualified public upgrade.
...
Version 0.1.10 is the current public-installation prerelease. Its [qualification](evals/history/2026-09-25-v010/qualification.json) records public asset, owner install/update/rollback/uninstall/reactivation and repair checks, one installed Agent coding task passing its independent grader, and failed UI task completion. An [unreleased guard candidate](evals/history/2026-09-25-guard-candidate/qualification.json) refused a direct process-name kill before native shell execution, but that result does not qualify the UI task or the public release. The [earlier v0.1.9 qualification](evals/history/2026-09-22-production/qualification.json) records broader mechanism checks and failed application acceptance. These results do not prove clean non-owner installation, sustained engineering quality or frontier parity.
