...
The unreleased source candidate adds a bounded read-only view of current Git state and up to four files named in the native checkpoint. It records a Git/content fingerprint at compaction and warns when that checkpoint's workspace state has changed, including after a restart. New checkpoints also bind retrieval to a hash of the native compaction event's summary, so a later ordinary message cannot impersonate that checkpoint; preexisting saved sessions use a legacy wrapper check. It keeps the first and three latest user requests in private state and flags a checkpoint that plainly denies an existing task. The observed-check ledger retains a bounded runner label, exit code and canonical failure reason for recognized checks, without copying raw commands or output into the prompt. Its coverage remains partial and does not prove task acceptance. The request record is intentionally bounded and may omit criteria; the complete native transcript stays on disk outside the prompt. A completed compaction is only a structural event, not proof that its summary is accurate. Public v0.1.10 retains the previous behavior; long-workflow qualification has not passed.
