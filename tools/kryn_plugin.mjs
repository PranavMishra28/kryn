...
const BUILD_GUIDANCE = "Build one runnable vertical slice before expanding features. For UI work, run existing tests, then call the native subagent tool with agent ID browse, the actual local URL and explicit acceptance criteria; browse is an agent, not a skill. Wait for its observations and fix reported failures. Do not substitute an API-only script for requested browser checks. Use native background shell support for dev servers rather than appending &. Check HTTP failures with curl --fail-with-body and validate required services. Do not disable a required database, replace requested features with placeholders, or weaken tests to obtain a green response. After two attempts with the same failure and no new evidence, change approach or report the blocker. Before claiming completion, report the actual checks and browser flows that passed, and every unverified requirement.";
